...
- [feature] Help Genie — Phase 2: Haiku conversational fallback. Layer Claude Haiku on top of the static help genie for questions the knowledge base can't answer. Serverless endpoint (Vercel function) with the app/feature context cached; the static search runs first and only falls through to Haiku on a no-match. Read-only by construction (no tools, no DB, text-out only). Near-$0 (cents/month) with an Anthropic Console spend cap + per-user rate limit.
